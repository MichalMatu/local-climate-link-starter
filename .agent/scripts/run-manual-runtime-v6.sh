...
# Rebuild the candidate deterministically from the approved baseline.
...
new = '''text = read(generate)
start = text.index('if(R.m){Shelly.call("Switch.Set"')
end = text.index('}if(e){', start) + 1
text = text[:start] + 'if(R.m)return Shelly.call("Switch.Set",{id:C.i,on:false});' + text[end:]
write(generate, text)'''
if old not in s:
    raise SystemExit('v6 could not locate the fragile v5 sw replacement block')
p.write_text(s.replace(old, new, 1))
PY
python3 /tmp/manual-v5.py

# Runtime mode is owned by Script.Eval, not duplicated into diagnostics.
...
# 1. Semantics first: live telemetry in MANUAL, no automatic output, clean AUTO re-entry.
pnpm --filter @lcl/script-generator exec vitest run \
  src/__tests__/manual-runtime.test.ts \
  src/__tests__/runtime-matrix.test.ts

# 2. Generator contract + strict byte budgets. Snapshot update is accepted only if those tests pass.
pnpm --filter @lcl/script-generator exec vitest run src/__tests__/generator.test.ts -u

# 3. App-side transport/status/safety/recovery/UI behaviour.
...
echo MANUAL_RUNTIME_V6_SHA=$(git rev-parse HEAD)
