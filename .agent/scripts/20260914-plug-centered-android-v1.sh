...
TMP=/tmp/lcl-plug-centered-55ec920a
...
"$ADB" -s "$SERIAL" shell uiautomator dump /sdcard/lcl-plug-centered.xml >/dev/null
"$ADB" -s "$SERIAL" exec-out cat /sdcard/lcl-plug-centered.xml > "$TMP/root-plugs.xml"
PID="$("$ADB" -s "$SERIAL" shell pidof "$PACKAGE" | tr -d '\r')"
[[ -n "$PID" ]] || { echo 'App process missing after cold start'; exit 7; }
...
ARTIFACT_WORKTREE=/tmp/lcl-plug-centered-artifact-ref
