...
if old not in s:
    raise SystemExit('v7 could not locate R.md -> R.m replacement')
...
# 1) Runtime semantics before snapshot acceptance.
pnpm --filter @lcl/script-generator exec vitest run \
  src/__tests__/manual-runtime.test.ts \
  src/__tests__/runtime-matrix.test.ts

# 2) Generator invariants including strict byte budgets; update snapshots only if these pass.
...
echo MANUAL_RUNTIME_V7_SHA=$(git rev-parse HEAD)
