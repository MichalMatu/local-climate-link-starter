...
git show origin/agent-control:.agent/scripts/run-manual-runtime-v14.sh > /tmp/run-manual-runtime-v21-driver.sh

cat > /tmp/manual-runtime-v21-adjust.py <<'PY'
...
'''
if marker not in s:
    raise SystemExit('v21 could not locate resource diagnostics marker')
...
if old not in s:
    raise SystemExit('v21 could not locate diagnosticResourceMutation')
...
if s.count(run_needle) != 1:
    raise SystemExit(f'v21 expected one V13 execution line, found {s.count(run_needle)}')
append_patch = r'''
cat >> /tmp/manual-runtime-v13.py <<'PYV21'
...
if s.count(old) != 1:
    raise SystemExit(f'v21 expected one controlRpcResult default, found {s.count(old)}')
...
new = """const mockShellyRpc = async (page: Page) => {
  ...
  let runtimeMode = 0;"""
if s.count(old) != 1:
    raise SystemExit(f'v21 expected one mockShellyRpc state block, found {s.count(old)}')
s = s.replace(old, new, 1)

old = "      params?: { id?: number; on?: boolean };"
new = "      params?: { id?: number; on?: boolean; code?: string };"
if s.count(old) != 1:
    raise SystemExit(f'v21 expected one responsive RPC params type, found {s.count(old)}')
s = s.replace(old, new, 1)
...
new = """      case 'Script.Eval': {
      ...
      case 'Script.Stop':
        scriptRunning = false;"""
if s.count(old) != 1:
    raise SystemExit(f'v21 expected one responsive Script.Stop block, found {s.count(old)}')
s = s.replace(old, new, 1)
p.write_text(s)
PYV21
...
s = s.replace('MANUAL_RUNTIME_V14_SHA', 'MANUAL_RUNTIME_V21_SHA')
p.write_text(s)
PY

python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/run-manual-runtime-v21-driver.sh')
s = p.read_text()
old = 'sh /tmp/run-manual-runtime-v14-expanded.sh\n'
new = 'python3 /tmp/manual-runtime-v21-adjust.py\nsh /tmp/run-manual-runtime-v14-expanded.sh\n'
if s.count(old) != 1:
    raise SystemExit(f'v21 expected one V14 execution line, found {s.count(old)}')
p.write_text(s.replace(old, new, 1))
PY

sh /tmp/run-manual-runtime-v21-driver.sh
