...
BASE='670fbc7aeb1e6907d6edf8479a2088118ab42d66'
...
cat > apps/mobile/src/screens/hardware-setup/pages/useShellySetupFeedback.ts <<'EOF'
...
s = s.replace("  const refreshShellyControl = flow.refreshShellyControl;\n", "")
s = s.replace("  const acknowledgeShellyControlFeedback = flow.acknowledgeShellyControlFeedback;\n", "")
start_marker = "  const pollBleDiscoveryRef = useRef<() => void>(() => undefined);\n"
end_marker = "  const checkShelly = () => {\n"
start = s.index(start_marker)
end = s.index(end_marker, start)
replacement = """  const { resetBleStopError } = useShellySetupFeedback({
    flow,
    isBleScanModalOpen,
    pushToast,
    t
...
s = s.replace("    shownBleStopErrorRef.current = null;\n", "    resetBleStopError();\n")
p.write_text(s)
PY
...
git diff --check
PAGE_LINES=$(wc -l < apps/mobile/src/screens/hardware-setup/pages/ShellySetupPage.tsx | tr -d ' ')
HOOK_LINES=$(wc -l < apps/mobile/src/screens/hardware-setup/pages/useShellySetupFeedback.ts | tr -d ' ')
echo SHELLY_SETUP_PAGE_LINES="$PAGE_LINES"
echo SHELLY_SETUP_FEEDBACK_LINES="$HOOK_LINES"
test "$PAGE_LINES" -lt 700
test "$HOOK_LINES" -lt 180

git add \
  apps/mobile/src/screens/hardware-setup/pages/useShellySetupFeedback.ts \
  apps/mobile/src/screens/hardware-setup/pages/ShellySetupPage.tsx
git commit -m 'Extract Shelly setup feedback orchestration'
git push origin HEAD:"$BRANCH"

echo STAGE8C2_SHA=$(git rev-parse HEAD)
echo STAGE8C2_PARENT=$(git rev-parse HEAD^)
echo STAGE8C2_CHECK=1
echo STAGE8C2_E2E=1
echo STAGE8C2_PAGE_LINES="$PAGE_LINES"
echo STAGE8C2_HOOK_LINES="$HOOK_LINES"
test -z "$(git status --porcelain)"
