...
git show origin/agent-control:.agent/scripts/run-manual-runtime-v16.sh > /tmp/run-manual-runtime-v17.sh

python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/run-manual-runtime-v17.sh')
s = p.read_text()
needle = "insert = '''# Register the device-status RPC centrally instead of scattering an untyped method name.\n"
if needle not in s:
    raise SystemExit('v17 could not locate v16 insertion block')
...
s = s.replace('MANUAL_RUNTIME_V16_SHA', 'MANUAL_RUNTIME_V17_SHA')
p.write_text(s)
PY

sh /tmp/run-manual-runtime-v17.sh
