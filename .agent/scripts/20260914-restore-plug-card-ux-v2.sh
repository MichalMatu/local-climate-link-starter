...
CHECKPOINT=/Users/michal/agent-workspace/repos/local-climate-link-starter/checkpoints/20260914-restore-plug-card-ux-v1/1789365254455689000-task-exit
cd "$REPO"
...
[ -f "$CHECKPOINT/tracked.patch" ] || { echo "Checkpoint patch missing"; exit 3; }
...
for p in \
  ...
  [ -f "$src" ] || { echo "Checkpoint file missing: $src"; exit 4; }
  mkdir -p "$(dirname "$p")"
  cp "$src" "$p"
done
