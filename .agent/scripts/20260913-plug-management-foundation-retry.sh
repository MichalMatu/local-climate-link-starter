...
# Recover only the exact files left dirty by the failed prior task.
git fetch --prune origin "$BRANCH" agent-control
git restore --source="origin/$BRANCH" -- \
...
Path('/tmp/lcl-plug-management-foundation-fixed.sh').write_text(s)
PY
bash /tmp/lcl-plug-management-foundation-fixed.sh
