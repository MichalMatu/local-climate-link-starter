...
git show origin/agent-control:.agent/scripts/20260913-rule-runtime-foundation-v3.sh > /tmp/lcl-rule-runtime-foundation-v6-base.sh
python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/lcl-rule-runtime-foundation-v6-base.sh')
s = p.read_text()
start_marker = "cat > apps/mobile/src/flows/rules/timeRuntime.test.ts <<'EOF'\n"
end_marker = "\nEOF\ncat > apps/mobile/src/flows/rules/climateRuntime.test.ts <<'EOF'\n"
start = s.find(start_marker)
end = s.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit('timeRuntime.test heredoc markers missing')
replacement = r'''cat > apps/mobile/src/flows/rules/timeRuntime.test.ts <<'EOF'
...
p.write_text(s)
PY
bash /tmp/lcl-rule-runtime-foundation-v6-base.sh
