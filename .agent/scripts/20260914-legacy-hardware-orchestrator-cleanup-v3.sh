...
saved_start = text.index('const checkSavedShellyCardFeedback = async () => {')
saved_end = text.index('\n\nconst checkTokenizedCssCoverage = async () => {', saved_start)
text = text[:saved_start] + text[saved_end + 2:]
...
! grep -q "checkSavedShellyCardFeedback" scripts/quality/ux-gate.mjs
