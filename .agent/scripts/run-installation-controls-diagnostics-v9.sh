...
git show origin/agent-control:.agent/scripts/run-installation-controls-diagnostics-v1.sh > /tmp/run-installation-controls-diagnostics-v9-inner.sh

python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/run-installation-controls-diagnostics-v9-inner.sh')
s = p.read_text()
replacements = {
...
sh /tmp/run-installation-controls-diagnostics-v9-inner.sh
...
s = p.read_text()
old = "  await expect(manual).toHaveAttribute('aria-pressed', 'true');\n  await expect(page.getByText('OFF', { exact: true })).toBeVisible();"
new = "  await expect(manual).toHaveAttribute('aria-pressed', 'true');\n  await expect(page.getByRole('button', { name: 'OFF', exact: true })).toHaveAttribute(\n    'aria-pressed',\n    'true'\n  );"
if s.count(old) != 1:
    raise SystemExit(f'responsive MANUAL/OFF locator marker mismatch: {s.count(old)}')
p.write_text(s.replace(old, new, 1))
PY
