...
git show origin/agent-control:.agent/scripts/run-setup-page-state-hardening-v1-20260911.sh > /tmp/setup-page-state-hardening-v2-inner.sh
python3 - <<'PY'
from pathlib import Path
p=Path('/tmp/setup-page-state-hardening-v2-inner.sh')
s=p.read_text()
...
p.write_text(s)
PY
sh /tmp/setup-page-state-hardening-v2-inner.sh
