...
git show origin/agent-control:.agent/scripts/run-manual-runtime-v14.sh > /tmp/run-manual-runtime-v19-driver.sh

cat > /tmp/manual-runtime-v19-dashboard.py <<'PY'
...
if s.count(old) != 1:
    raise SystemExit(f'v19 expected one controlRpcResult default, found {s.count(old)}')
p.write_text(s.replace(old, new, 1))
PY

cat > /tmp/manual-runtime-v19-adjust.py <<'PY'
from pathlib import Path
...
'''
if marker not in s:
    raise SystemExit('v19 could not locate resource diagnostics marker')
...
if old not in s:
    raise SystemExit('v19 could not locate diagnosticResourceMutation')
s = s.replace(old, new, 1)

needle = "pnpm exec prettier --write \\\n"
if needle not in s:
    raise SystemExit('v19 could not locate prettier stage')
s = s.replace(needle, 'python3 /tmp/manual-runtime-v19-dashboard.py\n\n' + needle, 1)
s = s.replace('MANUAL_RUNTIME_V14_SHA', 'MANUAL_RUNTIME_V19_SHA')
p.write_text(s)
PY

python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/run-manual-runtime-v19-driver.sh')
s = p.read_text()
old = 'sh /tmp/run-manual-runtime-v14-expanded.sh\n'
new = 'python3 /tmp/manual-runtime-v19-adjust.py\nsh /tmp/run-manual-runtime-v14-expanded.sh\n'
if old not in s:
    raise SystemExit('v19 could not locate v14 execution')
p.write_text(s.replace(old, new, 1))
PY

sh /tmp/run-manual-runtime-v19-driver.sh
