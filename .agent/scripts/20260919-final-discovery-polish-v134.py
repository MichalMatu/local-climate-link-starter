...
# Remove legacy discovery layout overrides superseded by device-discovery-card.
replace_once(
    theme,
...
# Shelly regression checks: spinner/busy state, shared action contract, primary URL identity.
replace_once(
    tests,
...
replace_once(
    ...
    '''    expect(within(scannedRow!).getAllByRole('textbox')).toHaveLength(1);\n    expect(\n      within(scannedRow!).getByText('http://192.168.0.20/')\n    ).toHaveClass('device-discovery-card__identity');\n    expect(within(scannedRow!).getByText('S3PL-00112EU, gen 3')).toBeVisible();'''
)

# BLE regression checks: same identity/action semantics and in-button active spinner.
replace_once(
    tests,
...
print("Applied corrected final discovery polish")
