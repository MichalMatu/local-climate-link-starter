...
git show origin/agent-control:.agent/scripts/run-navigation-ux-consistency-v2.sh > /tmp/run-navigation-ux-consistency-v4-inner.sh

cat > /tmp/navigation-ux-v4-patch.py <<'PY'
from pathlib import Path
import re

# Floating app-level Settings trigger is obsolete after bottom-nav unification.
...
python3 - <<'PY'
from pathlib import Path
p = Path('/tmp/run-navigation-ux-consistency-v4-inner.sh')
s = p.read_text()
...
sh /tmp/run-navigation-ux-consistency-v4-inner.sh
