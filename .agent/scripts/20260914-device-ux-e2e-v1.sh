...
cat > apps/mobile/e2e/device-ux.spec.ts <<'EOF'
...
const seedRegistries = async (
  ...
  await page.addInitScript(({ plugs, sensors, rules }) => {
...
test('restored Shelly card keeps rich telemetry, menu and rule-aware detail UX', async ({
  ...
  await expect(dialog.getByRole('button', { name: 'ON' })).toBeDisabled();
...
test('thermometer card keeps polished metrics and rule usage while schedule uses one-row weekdays and wheel picker', async ({
  ...
  await expect(sensorCard.locator('.sensor-card-details')).toContainText(sensor.runtimeAddress);
  await expect(sensorCard.locator('.sensor-rule-usage__link')).toContainText(climateRule.name);
  ...
  const [onBox, offBox] = await Promise.all([onButton.boundingBox(), offButton.boundingBox()]);
...
pnpm exec eslint apps/mobile/e2e/device-ux.spec.ts
pnpm --filter @lcl/mobile typecheck
pnpm exec playwright test -c apps/mobile/playwright.config.ts device-ux.spec.ts
