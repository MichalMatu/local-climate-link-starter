...
path = Path('apps/mobile/src/__tests__/rule-editor.test.tsx')
text = path.read_text()
old = "screen.getByLabelText('Nazwa')"
new = "screen.getByRole('textbox', { name: /Nazwa/ })"
count = text.count(old)
if count < 1:
    raise SystemExit('Expected at least one exact name label query')
text = text.replace(old, new)
if old in text:
    raise SystemExit('Exact name label query still present after replacement')
path.write_text(text)
print(f'Replaced {count} exact name label queries')
PY
