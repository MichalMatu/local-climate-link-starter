...
The setup UI supports Polish, English, German, Spanish, French, Italian, and
Brazilian Portuguese from the same typed i18n key tree. The app selects the
language from the system browser/webview language. Unsupported languages fall
back to English, and generic Portuguese tags resolve to `pt-BR`. There is no
manual language switch in the MVP UI. New languages should be added by creating
another locale file under `apps/mobile/src/app/locales/`, registering it in
`apps/mobile/src/app/i18n.ts`, and running the i18n tests that verify key parity
and block hardcoded Polish copy outside the locale files.
