...
Dedicated `readShellyScriptList` / `readShellyScriptCode` helpers own validated script-read RPC shapes. Install flow parses `Script.List`, reuses and backs up the existing Shelly Link script when found, uploads code with chunked `Script.PutCode`, enables run-on-boot, starts the script, and verifies `Script.GetStatus`.
