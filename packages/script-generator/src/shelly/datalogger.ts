...
export const SHELLY_DATALOGGER_SCRIPT_MAX_BYTES = 4_000;
...
function sm(d){if(!d||!Array.isArray(d.g)||!Array.isArray(d.y)||d.g.length<13)return null;var y=d.y,g=d.g,p=d.p,b,t,o;if(typeof y[1]==="number"&&y[1]>0){b=0;t=Math.floor(y[1]);}else if(typeof y[2]==="number"&&y[2]>=0){b=1;t=Math.floor(y[2]);}else return null;o=Array.isArray(p)&&typeof p[0]==="boolean"?p[0]:g[5];if(typeof o!=="boolean")return null;return{b:b,t:t,r:[sc(g[1],10),sc(g[2],10),sc(g[12],100),o?1:0,typeof g[6]==="string"?g[6].slice(0,16):null]};}
...
function ad(s){if(W){P=s;return;}if(!B)nb(s);if(B[2]!==s.b||s.t<B[3]){P=s;fl();return;}B[4].push([s.t-B[3],s.r[0],s.r[1],s.r[2],s.r[3],s.r[4]]);if(JSON.stringify(B).length>${LCL_HISTORY_MAX_VALUE_CHARS}){B[4].pop();P=s;fl();}}
function pl(){if(Q||W)return;Q=1;Shelly.call("Script.Eval",{id:C.s,code:"diag()"},function(r,e){Q=0;if(e||!r||typeof r.result!=="string"){E="source-read";return;}var d;try{d=JSON.parse(r.result);}catch(x){E="source-json";return;}var s=sm(d);if(!s){E="source-shape";return;}E="";ad(s);if(B&&up()-F>=C.f)fl();});}
