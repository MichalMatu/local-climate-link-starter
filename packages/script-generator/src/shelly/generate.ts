...
function parse(x){var d=sd(x);if(!d){R.ds="bm";return;}var t=null,h=null,b=null,o=1,l=lb(d),k,v;while(o<l){k=rb(d,o++);if(k==0)o++;else if(k==1)b=rb(d,o++);else if(k==12)o+=2;else if(k==2){v=r2(d,o,1);if(v==null){R.ds="bs";return;}t=v/100;o+=2;}else if(k==3){v=r2(d,o,0);if(v==null){R.ds="bs";return;}h=v/100;o+=2;}else if(k==46){h=rb(d,o++);if(h==null){R.ds="bs";return;}}else if(k==69){v=r2(d,o,1);if(v==null){R.ds="bs";return;}t=v/10;o+=2;}else{R.ds="bo";break;}}meas(t,h,b,x.rssi);}`;
...
function parse(x){var d=x.advData;if(!d){R.ds="ta";return;}var p=mf(d);if(p===null){R.ds="tm";return;}var lo=rb(d,p+1),hi=rb(d,p+2),h=rb(d,p+3),b=rb(d,p+4);if(lo===null||hi===null||h===null||b===null){R.ds="ts";return;}var raw=lo|(hi<<8);if(raw&32768)raw-=65536;var t=raw/10;if(h>100||t<-50||t>100){R.ds="tr";return;}meas(t,h,b,x.rssi);}`;
...
const renderMeasurementHelper = (config: ShellyThermostatConfig): string => {
  const commonDecision =
    'R.ds="ok";var T=th(t,h);R.eo=T.o;R.ef=T.f;R.vp=C.vp?vd(t,h):null;var go=C.d?v>T.o:v<T.o,stop=C.d?v<T.f:v>T.f,gr=C.d?"ab":"bl",sr=C.d?"bl":"ab";if(go){R.nh++;R.fh=0;if(R.nh<C.h){R.rs=gr+"h";return;}sw(true,gr,false);return;}if(stop){R.fh++;R.nh=0;sw(false,sr,false);return;}R.nh=0;R.fh=0;R.rs="ib";';
...
function meas(t,h,b,r){var n=nw(),p=t!=null||h!=null,ct=C.m?h:t;R.r=r;if(b!=null)R.b=b;if(t!=null){R.t=t;R.tt=n;}if(h!=null){R.h=h;R.ht=n;}if(!p){R.ds="cv";return;}var tf=fr(R.tt,n)?R.t:null,hf=fr(R.ht,n)?R.h:null,v=C.m?hf:tf;R.cv=v;if(C.vp){if(v==null||tf==null||hf==null){R.ds="cv";return;}t=tf;h=hf;}else{if(ct==null){R.ds=v==null?"cv":"pt";return;}if(v==null){R.ds="cv";return;}t=tf;h=hf;}R.ls=n;${commonDecision}}`;
  ...
  if (!config.rule.vpdAssist.enabled) {
    return `function meas(t,h,b,r){var n=nw(),v=C.m?h:t;R.r=r;R.cv=v;if(b!=null)R.b=b;if(v==null){R.ds="cv";return;}R.ls=n;R.t=t;R.h=h;${commonDecision}}`;
  }

  return `function fr(x,n){return x!==null&&n-x<=Math.min(${COMPOSITE_MEASUREMENT_WINDOW_MS},C.s);}
function meas(t,h,b,r){var n=nw(),p=t!=null||h!=null;R.r=r;if(b!=null)R.b=b;if(t!=null){R.t=t;R.tt=n;}if(h!=null){R.h=h;R.ht=n;}if(!p){R.cv=null;R.ds="cv";return;}t=fr(R.tt,n)?R.t:null;h=fr(R.ht,n)?R.h:null;var v=C.m?h:t;R.cv=v;if(v==null||!fr(R.tt,n)||!fr(R.ht,n)){R.ds="cv";return;}R.ls=n;${commonDecision}}`;
};
...
function sw(o,rs,f){var n=nw(),ch=R.on!==o;if(!f&&!ch){R.rs=rs;return;}if(o&&!f&&n-R.lc<C.c){R.rs="mc";return;}Shelly.call("Switch.Set",{id:C.i,on:o},function(r,e){if(e!==0){R.rs="se";Shelly.call("Switch.Set",{id:C.i,on:false});R.on=false;return;}R.on=o;R.rs=rs;if(ch)R.lc=n;R.os=o?n:null;});}
