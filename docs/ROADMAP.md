...
The mobile draft/edit boundary uses normalized physical BLE `runtimeAddress` as the canonical thermometer identity. Phone discovery, Plug discovery, installed config and Load from Shelly converge on the same physical row. Recovered membership is tracked as transient provenance: ordinary edits preserve it, an explicit membership change stops silently carrying inherited additional sensors, and Load from Shelly remains the authoritative full-set replacement.
