...
| Final destructive `2.0.10` phone-alpha install | blocked     | Not executed in the continuation on 2026-09-10 because `adb devices -l` was empty; restarting the ADB server did not restore `RFCT70L7E8J`. The task stopped before uninstall/install, so no `2.0.10` device PASS is claimed yet.                                    |
