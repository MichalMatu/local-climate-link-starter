...
shelly-soak-start: ## Start long Shelly soak logging in the background. Requires SHELLY_URL.
	...
	@if [ -f "$(SOAK_PID_FILE)" ] && kill -0 "$$(cat "$(SOAK_PID_FILE)")" 2>/dev/null; then \
		...
		SHELLY_URL="$$SHELLY_URL" SCRIPT_ID="$(SCRIPT_ID)" SOAK_OUT_FILE="$$out_file" SOAK_SUMMARY_FILE="$$summary_file" SOAK_INTERVAL_MS="$(SOAK_INTERVAL_MS)" SOAK_RPC_TIMEOUT_MS="$(SOAK_RPC_TIMEOUT_MS)" SOAK_DURATION_MS="$(SOAK_DURATION_MS)" SOAK_CYCLE_RELAY="$(SOAK_CYCLE_RELAY)" SOAK_CYCLE_PERIOD_MS="$(SOAK_CYCLE_PERIOD_MS)" SOAK_CYCLE_MARGIN="$(SOAK_CYCLE_MARGIN)" SOAK_CYCLE_MIN_CHANGE_MS="$(SOAK_CYCLE_MIN_CHANGE_MS)" SOAK_CYCLE_MAX_ON_MS="$(SOAK_CYCLE_MAX_ON_MS)" SOAK_CYCLE_CONSECUTIVE_HITS="$(SOAK_CYCLE_CONSECUTIVE_HITS)" SOAK_FINAL_OFF="$(SOAK_FINAL_OFF)" SOAK_STOP_SCRIPT_ON_FINISH="$(SOAK_STOP_SCRIPT_ON_FINISH)" nohup "$$node_bin" "$$tsx_cli" "$(ROOT_DIR)/scripts/hardware/shelly-soak-logger.ts" >"$$log_file" 2>"$$error_log_file" & \
		pid=$$!; \
		echo "$$pid" >"$(SOAK_PID_FILE)"; \
		printf 'SOAK_OUT_FILE=%s\nSOAK_SUMMARY_FILE=%s\nSOAK_LOG_FILE=%s\nSOAK_ERROR_LOG_FILE=%s\nSOAK_CYCLE_RELAY=%s\nSOAK_CYCLE_PERIOD_MS=%s\nSOAK_CYCLE_MARGIN=%s\nSOAK_CYCLE_MIN_CHANGE_MS=%s\nSOAK_CYCLE_MAX_ON_MS=%s\nSOAK_CYCLE_CONSECUTIVE_HITS=%s\nSOAK_FINAL_OFF=%s\nSOAK_STOP_SCRIPT_ON_FINISH=%s\n' "$$out_file" "$$summary_file" "$$log_file" "$$error_log_file" "$(SOAK_CYCLE_RELAY)" "$(SOAK_CYCLE_PERIOD_MS)" "$(SOAK_CYCLE_MARGIN)" "$(SOAK_CYCLE_MIN_CHANGE_MS)" "$(SOAK_CYCLE_MAX_ON_MS)" "$(SOAK_CYCLE_CONSECUTIVE_HITS)" "$(SOAK_FINAL_OFF)" "$(SOAK_STOP_SCRIPT_ON_FINISH)" >"$(SOAK_RUN_FILE)"; \
